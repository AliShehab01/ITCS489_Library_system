...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_res'])) {
    ...
    if ($userId && $bookId) {
        $chk = $conn->prepare("SELECT 1 FROM reservations WHERE user_id=? AND book_id=? AND status IN ('active','notified')");
        $chk->bind_param("ii", $userId, $bookId);
        $chk->execute();
        if ($chk->get_result()->fetch_row()) {
            $msg = "User already has an active reservation for this book.";
        } else {
            $ins = $conn->prepare("INSERT INTO reservations (user_id, book_id) VALUES (?, ?)");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cancel_res'])) {
    ...
    if ($resId) {
        $upd = $conn->prepare("UPDATE reservations SET status='cancelled' WHERE reservation_id=?");
      ...
      <select name="book_id" class="form-select" required>
        ...
        <?php foreach ($unavailableBooks as $b): ?>
          <option value="<?= (int)$b['id'] ?>">
            <?= htmlspecialchars($b['title']) ?> (<?= htmlspecialchars($b['availability']) ?>, qty: <?= (int)$b['quantity'] ?>)
          </option>
        <?php endforeach; ?>
      </select>
    </div>
  ...
  <?php else: ?>
    <div class="table-responsive">
      <table class="table table-sm bg-white align-middle">
        <thead><tr><th>Book</th><th>User</th><th>Status</th><th>Reserved At</th><th></th></tr></thead>
        <tbody>
            ...
            <tr>
              ...
              <td>
                <form method="POST" class="d-inline">
                  ...
                  <button class="btn btn-sm btn-outline-danger" name="cancel_res" value="1">Cancel</button>
                </form>
              </td>
            </tr>
          <?php endforeach; ?>
